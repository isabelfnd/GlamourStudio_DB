...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-05-2026 a las 17:15:25
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_registrar_cita` (IN `p_id_cliente` INT, IN `p_id_estilista` INT, IN `p_fecha` DATETIME)   BEGIN
    INSERT INTO cita(id_cliente, id_estilista, fecha, estado)
    VALUES(p_id_cliente, p_id_estilista, p_fecha, 'Pendiente');
END$$

DROP PROCEDURE IF EXISTS `sp_registrar_cliente`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_registrar_cliente` (IN `p_nombre1` VARCHAR(50), IN `p_nombre2` VARCHAR(50), IN `p_apellido1` VARCHAR(50), IN `p_apellido2` VARCHAR(50), IN `p_telefono` VARCHAR(20), IN `p_email` VARCHAR(100), IN `p_direccion` VARCHAR(255))   BEGIN
    INSERT INTO cliente(id_categoria, nombre1, nombre2, apellido1, apellido2, telefono, email, direccion, fecha_registro)
    VALUES(1, p_nombre1, p_nombre2, p_apellido1, p_apellido2, p_telefono, p_email, p_direccion, NOW());
END$$
...
INSERT INTO `cliente` (`id_cliente`, `id_categoria`, `nombre1`, `nombre2`, `apellido1`, `apellido2`, `telefono`, `email`, `direccion`, `fecha_registro`) VALUES
(1, 1, 'Diana', 'Marcela', 'López', 'Erazo', '3159876543', 'diana@email.com', 'Carrera 9 #4-50', '2026-05-22 11:46:15');
